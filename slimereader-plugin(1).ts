...
function init() {
    $ui.register((ctx) => {
        ...
        const currentVolId = ctx.state<string>("");
        const pageContent  = ctx.state<string>("");
        const isLoading    = ctx.state<boolean>(false);

        // ── Webview (screen slot = its own page in the sidebar) ──────────────
        ...
        webview.channel.sync("volumes",      ctx.state(VOLUMES));
        webview.channel.sync("currentVolId", currentVolId);
        ...
        // ── Tray icon — navigates directly to the webview screen page ────────
        ...
        tray.onClick(() => {
            console.log("[slime-reader] tray clicked — navigating to webview screen");
...
#reader-wrap { padding: 32px 24px 80px; max-width: 760px; margin: 0 auto; font-size: 18px; line-height: 1.85; font-family: 'Georgia', serif; }
...
function goHome() {
    window.webview.send("go-home", null);
}
...
function buildGrid(vols) {
    var g = document.getElementById("grid");
    g.innerHTML = vols.map(function(v) {
        return '<div class="card" onclick="loadVolume(\'' + v.id + '\')">'
...
if (window.webview) {
    window.webview.on("volumes", function(vols) {
        _vols = vols;
        buildGrid(vols);
    });

    window.webview.on("currentVolId", function(id) {
        if (!id) { showHome(); return; }
        var vol = _vols.find(function(v) { return v.id === id; });
        showReader(vol ? vol.name : id);
    });

    window.webview.on("isLoading", function(loading) {
        if (loading) {
    ...
    window.webview.on("pageContent", function(html) {
        if (!html) return;
        document.getElementById("reader-wrap").innerHTML = html;
        document.getElementById("main").scrollTop = 0;
    });
}
</script>
